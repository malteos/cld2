...
Compared to GlotLID~v3 and OpenLID-v3, CLD2~extended achieves the highest micro~\fone{} (0.9263 vs.\ 0.8015 and 0.7683) while being 76$\times$ and 27$\times$ faster, respectively.
...
To contextualize our results, we evaluate two state-of-the-art open-source LID models on the same CommonLID benchmark: GlotLID~v3~\citep{kargaran-etal-2023-glotlid} and OpenLID-v3~\citep{burchell-etal-2023-open}, both fastText-based models.

\paragraph{Accuracy.}
Table~\ref{tab:baselines} compares accuracy metrics.
CLD2 extended achieves macro~\fone{} within 0.6\% of GlotLID (0.7298 vs.\ 0.7354), while substantially outperforming it on micro~\fone{} (0.9263 vs.\ 0.8015).
The micro~\fone{} gap is driven by GlotLID's failure on several high-volume languages: Uzbek (\fone{}=0, 43K~samples), Malagasy (\fone{}=0, 2.2K~samples), and Estonian (\fone{}=0, 659~samples) all score zero---likely due to label mismatches between GlotLID's training labels and CommonLID's ground truth---whereas CLD2 handles these correctly.
OpenLID-v3 scores lowest overall, with 18~languages at \fone{}=0 including Breton, Aragonese, Bikol, and Frisian, which it does not cover.
...
\textbf{CLD2 extended} & \textbf{0.7298} & \textbf{0.9263} & \textbf{100\%} \\
GlotLID v3 & 0.7354 & 0.8015 & 100\% \\
\bottomrule
\end{tabular}
\caption{Accuracy comparison on CommonLID (373K samples, 89 languages). CLD2 extended achieves the highest micro~\fone{} and is within 0.6\% of GlotLID on macro~\fone{}. Bold marks best per column (excluding macro~\fone{} where GlotLID leads).}
\label{tab:baselines}
\end{table}

\paragraph{Speed.}
Table~\ref{tab:speed} reports prediction throughput measured on 50,000 CommonLID text samples (single-threaded, same hardware).
CLD2 is 76$\times$ faster than GlotLID and 27$\times$ faster than OpenLID-v3.
This is expected: CLD2 uses precomputed hash-table lookups on byte sequences with $O(n)$ complexity and small constants, while fastText models require tokenization and matrix multiplications.

On the HTML benchmark (10,000 Common Crawl pages including HTML parsing overhead), CLD2~extended processes all pages in 3,139\,ms---12\% faster than the 3,565\,ms vanilla CLD2 baseline, due to prior performance optimizations in the same branch.
...
The extended system matches GlotLID's macro~\fone{} (0.7298 vs.\ 0.7354) while achieving the highest micro~\fone{} (0.9263) at 76$\times$ the throughput.
